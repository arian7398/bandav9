...
function guardarDatos(datos) {
  try {
    ...
    //------------------------------------------------------------------------------------------------------------------
    // Validar la sesión y obtener el usuario
    const sessionData = validarSesion(sessionToken);
    if (!sessionData) {
      return {
        success: false,
    ...
    sheet.appendRow([
      ...
      datos.fiscalia,
      datos.fiscalacargo,
      sessionData.username,  // Añadir usuario creador---------------------------------------------------------------
      // Detalles del Caso
  ...
  try {
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i][0] === datos.id) {
        ...
        usuarioOriginal = data[i][2]; // Guardar el usuario original
        break;
      }
    }
    ...
    if (filaEncontrada > 0) {
      ...
      if (sessionData.role !== 'admin' && sessionData.username !== usuarioOriginal) {
        return {
          success: false,
      ...
      sheet.getRange(filaEncontrada, 4).setValue(datos.fiscalia);
      sheet.getRange(filaEncontrada, 5).setValue(datos.fiscalacargo);
      sheet.getRange(filaEncontrada, 6).setValue(datos.unidadInteligencia);
      sheet.getRange(filaEncontrada, 7).setValue(datos.instructorCargo);
      sheet.getRange(filaEncontrada, 8).setValue(datos.formaInicio);
      sheet.getRange(filaEncontrada, 9).setValue(datos.carpetaFiscal);
      sheet.getRange(filaEncontrada, 10).setValue(datos.fechaHecho);
      sheet.getRange(filaEncontrada, 11).setValue(datos.fechaIngresoFiscal);
      ...
      sheet.getRange(filaEncontrada, 15).setValue(funcionEjerce); // Nuevo campo
      sheet.getRange(filaEncontrada, 16).setValue(tipoEmpresa);   // Nuevo campo
      sheet.getRange(filaEncontrada, 17).setValue(datos.delitos);
      sheet.getRange(filaEncontrada, 18).setValue(datos.lugarHechos);
      sheet.getRange(filaEncontrada, 19).setValue(denunciadosTexto);
      sheet.getRange(filaEncontrada, 20).setValue(datos.datosInteresDenunciado);
      sheet.getRange(filaEncontrada, 21).setValue(datos.nombreBandaCriminal);
      sheet.getRange(filaEncontrada, 22).setValue(datos.cantidadMiembrosBanda);
      sheet.getRange(filaEncontrada, 23).setValue(datos.modalidadViolencia);
      sheet.getRange(filaEncontrada, 24).setValue(datos.modalidadAmenaza);
      sheet.getRange(filaEncontrada, 25).setValue(datos.atentadosCometidos);
      sheet.getRange(filaEncontrada, 26).setValue(instrumentosTexto);
      sheet.getRange(filaEncontrada, 27).setValue(datos.formaPago);
      sheet.getRange(filaEncontrada, 28).setValue(cuentasPagoTexto);
      sheet.getRange(filaEncontrada, 29).setValue(imeiTexto);
      sheet.getRange(filaEncontrada, 30).setValue(titularesTexto);
      sheet.getRange(filaEncontrada, 31).setValue(datos.tipoPago);
      sheet.getRange(filaEncontrada, 32).setValue(datos.montoSolicitado);
      sheet.getRange(filaEncontrada, 33).setValue(datos.montoPagado);
      sheet.getRange(filaEncontrada, 34).setValue(datos.numeroPagos);
      sheet.getRange(filaEncontrada, 35).setValue(datos.tipoPagoOtros);
      sheet.getRange(filaEncontrada, 36).setValue(datos.sumillaHechos);
      sheet.getRange(filaEncontrada, 37).setValue(datos.observaciones);
...
function obtenerRegistros(sessionToken) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      const usuarioCreador = row[2]; // Usuario creador está en la columna 3
      
     // Filtrar por usuario: solo mostrar registros propios o todos si es admin
      if (sessionData.role === 'admin' || usuarioCreador === sessionData.username) {
      ...
      var registro = {
        id: row[0],
        fechaRegistro: row[1],
        usuarioCreador: row[2], // Incluir el usuario creador en los datos
        ...
        // Detalles del Caso
        unidadInteligencia: row[5],
        instructorCargo: row[6],
        formaInicio: row[7],
        carpetaFiscal: row[8],
        fechaHecho: row[9],
        fechaIngresoFiscal: row[10],
        ...
        // Nuevos campos
        funcionAgraviado: row[13] || '',
        tipoEmpresa: tipoEmpresaTexto,
        empresas: empresas,

        cuentasPago: (row[28] && typeof row[28] === 'string') ? 
        row[28].split('\n').map(function(s) { return s.trim(); }) : [],
        
        // Información del Denunciado
        delitos: row[15],
        lugarHechos: row[16],
        denunciadosAlias: (row[17] && typeof row[17] === 'string') ? 
          row[17].split(', ').map(function(nombre) {
            return {id: Utilities.getUuid(), nombre: nombre.trim()};
          }) : [],
        ...
        datosInteresDenunciado: (() => {
          ...
          if (typeof row[18] === 'string') {
            return row[18].split(', ').map(item => item.trim()).filter(Boolean);
          }
        ...
        nombreBandaCriminal: row[19],
        cantidadMiembrosBanda: row[20],
        modalidadViolencia: row[21],
        modalidadAmenaza: row[22],
        atentadosCometidos: row[23],

        // Instrumentos y Métodos de Extorsión
        instrumentosExtorsion: (row[24] && typeof row[24] === 'string') ? 
          row[24].split(', ').map(function(s) { return s.trim(); }) : [],
        formaPago: row[25],
        numerosTelefonicos: (row[26] && typeof row[26] === 'string') ? 
          row[26].split(', ').map(function(s) { return s.trim(); }) : [],
        imeisTelefonicos: (row[27] && typeof row[27] === 'string') ? 
          row[27].split(', ').map(function(s) { return s.trim(); }) : [],
        cuentaPago: row[28],
        titularesPago: (row[29] && typeof row[29] === 'string') ? 
         row[29].split(', ').map(function(s) { return s.trim(); }) : [],
      
        // Datos de Interés de Pagos
        tipoPago: row[30],
        montoSolicitado: row[31],
        montoPagado: row[32],
        numeroPagos: row[33],
        tipoPagoOtros: row[34], 
        
        // Sumilla y Observaciones
        sumillaHechos: row[35],
        observaciones: row[36]
      };
...
function eliminarRegistro(id, sessionToken) {
  try {
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i][0] === id) {
        filaEncontrada = i + 1; // +1 porque las filas en Sheets empiezan en 1
        usuarioCreador = data[i][2]; // Usuario creador está en la columna 3
        break;
      }
    }
    ...
    if (filaEncontrada > 0) {
      ...
      if (sessionData.role !== 'admin' && sessionData.username !== usuarioCreador) {
        return {
          success: false,
